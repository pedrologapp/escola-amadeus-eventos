...
import { SERIES_DISPONIVEIS, TURMAS_DISPONIVEIS } from "@/lib/constants";

/**
 * Extrai dados estruturados de eventos a partir de texto ou PDF do aviso da escola.
 * Usa Claude Sonnet 4.6 com tool_use pra forçar JSON estrito, e prompt caching
 * pra reduzir custo a partir da 2ª extração.
 */

const SYSTEM_PROMPT = `Você é um assistente especializado em extrair informações estruturadas de comunicados/avisos enviados por escolas aos pais sobre eventos escolares.

CONTEXTO DA ESCOLA:
- Escola: Centro Educacional Amadeus (São Gonçalo do Amarante, RN)
- Séries disponíveis na escola: ${SERIES_DISPONIVEIS.join(", ")}
- Turmas: ${TURMAS_DISPONIVEIS.join(", ")}
...
8. Cor temática: sugira hex adequado ao tema:
   ...
   - Dia dos Pais → "#1B3B7C" (azul)
   - Festa Junina → "#F59E0B" (âmbar/laranja)
   - Natal → "#16A34A" (verde)
   - Formatura → "#7C3AED" (roxo)
   - Carnaval → "#F5A623" (amarelo)
   - Genérico → "#1B3B7C" (azul Amadeus)
9. series_permitidas: SOMENTE valores da lista acima. Se for aberto a todos, retorne null.
10. turmas_permitidas: SOMENTE A/B/C/D. Se for aberto a todas, retorne null.
11. infos_importantes: cada item DEVE ser uma frase completa, autocontida (ex: "Filhos da escola são isentos de pagamento.")
12. tipos_ingresso: SEMPRE pelo menos 1 item. Se o evento for gratuito, crie 1 tipo com preço 0.
13. LOTES — IMPORTANTE: Avisos de escola frequentemente cobram preços diferentes em datas diferentes. SEMPRE procure padrões como:
...
14. metodos_pagamento: padrão ["pix", "cartao"] a menos que o aviso restrinja
15. max_parcelas: padrão 3, a menos que mencione outro número
...
const extractEventoTool: Anthropic.Tool = {
  name: "extrair_evento",
  description: "Estrutura os dados extraídos do aviso de evento da escola.",
  input_schema: {
    type: "object",
    properties: {
      nome: {
      ...
      local: {
        type: ["string", "null"],
        description: "Local físico do evento (ex: 'Novo Auditório da Escola Amadeus').",
      },
      cor_tematica: {
        type: "string",
        pattern: "^#[0-9A-Fa-f]{6}$",
      ...
      series_permitidas: {
        ...
        items: {
          type: "string",
          enum: [...SERIES_DISPONIVEIS],
        },
        description:
          "Séries permitidas. Use null se aberto a todas. Use SOMENTE valores da lista.",
      },
      turmas_permitidas: {
        type: ["array", "null"],
        items: {
          type: "string",
          enum: [...TURMAS_DISPONIVEIS],
        },
        description:
          "Turmas permitidas (A/B/C/D). Use null se aberto a todas.",
      },
      metodos_pagamento: {
        type: "array",
        items: {
